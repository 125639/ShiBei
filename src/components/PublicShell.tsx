...
/** Firefly 风格的当月日历：纯服务端渲染的装饰组件，高亮今天与最近发布日 */
function MiniCalendar({ lastPublishedAt }: { lastPublishedAt: string | null }) {
  ...
  const publishedDay =
    lastPublished && lastPublished.getFullYear() === year && lastPublished.getMonth() === month
      ? lastPublished.getDate()
      : null;
  ...
  return (
    <section className="ff-widget ff-calendar-widget">
      <h2 className="ff-widget-title">
        <I18nText zh={`${year} 年 ${month + 1} 月`} en={`${year}-${String(month + 1).padStart(2, "0")}`} />
      </h2>
