...
- `next.config.mjs`：`/_next/static/*` 1 年 immutable，`/uploads/*` 1 小时缓存；启用 compress；`optimizePackageImports`。
...
2. `git clone` 仓库到 `~/ShiBei`（已存在则拉取最新代码；检测到未提交的本地修改会拒绝覆盖并提示先 stash/commit）
...
| 形态 | 启动命令 |
| --- | --- |
| 完整版 | `docker compose pull && docker compose up -d` |
| 后端 | `docker compose -f docker-compose.backend.yml pull && docker compose -f docker-compose.backend.yml up -d` |
| 前端 | `docker compose -f docker-compose.frontend.yml pull && docker compose -f docker-compose.frontend.yml up -d` |

> Compose 文件里同时保留了 `image:`（拉 Docker Hub 镜像）和 `build:`（本地构建），并已设置 `pull_policy: missing`。
>
> - 直接 `up -d` 时本地缺镜像会优先从 Docker Hub 拉取，**几分钟**就能起来；先 `pull` 可顺便获取最新版。
> - 加 `--build` 才会强制本地构建：低内存机器上 Playwright/Chromium 下载 + `next build` 容易 OOM，建议在 4 GB+ 的机器上构建并推到 Docker Hub。
...
迁移由 `prisma migrate deploy` 按记录顺序各执行一次；部分早期迁移带 `IF NOT EXISTS` 守卫，但不要手工重复执行已登记的 SQL。
...
1. **前端的「视频功能」开关没开**（最常见）：该开关**不随同步传递**且默认关闭，关闭时文章里的视频短代码被静默移除、前台完全不显示。到前端自己的 `/admin/settings?tab=media` 勾选「启用视频功能」。前端 `/admin/videos` 和 `/admin/sync` 在此状态下都会显示警告。
2. **外链 / EMBED**：原始 URL 可能限制 iframe，换一个能 embed 的来源。
3. **本地视频（LOCAL）**：自动同步**默认不带 mp4 文件**（保护小机）。文件未到达时视频会降级为指向来源页的链接卡片。要传文件：在后端 `/admin/sync` 下载「全量 ZIP（含本地视频）」，前端手动上传导入；前端默认只接受 64MB ZIP / 48MB 单文件，超过时先在前端 `.env` 调大 `SYNC_MAX_ZIP_MB` / `SYNC_MAX_FILE_MB`（注意 ZIP 会整体读入内存，别超过容器内存）。
4. 文件存在但播放失败：浏览器 devtools network 看 `/uploads/...` 的 HTTP 状态。`/uploads/[...path]/route.ts` 自带 Range 支持，断点续播没问题。
...
- **上传体积限制**：图片 8 MB、音乐 30 MB、视频 300 MB；ZIP 同步导入默认 frontend 64 MB / 其他形态 512 MB（`SYNC_MAX_ZIP_MB` 可调），按需改大但记得给反代也加 `client_max_body_size`。
